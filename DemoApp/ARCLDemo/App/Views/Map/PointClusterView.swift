...
struct PointClusterContentView: View {
    var body: some View {
...
struct PointClusteringExampleRepresentable: UIViewControllerRepresentable {
    func makeUIViewController(context: Context) -> PointClusteringExample {
        return PointClusteringExample() // Instantiate your UIViewController subclass
    }
    
    func updateUIViewController(_ uiViewController: PointClusteringExample, context: Context) {
...
final class PointClusteringExample: UIViewController, ExampleProtocol {
    ...
    func addPointClusters() {
        ...
        guard let url = Bundle.main.url(forResource: "predictions", withExtension: "geojson") else {
                print("GeoJSON file not found in the app bundle.")
                return
            }
        // Create a GeoJSONSource from the earthquake data URL.
        var source = GeoJSONSource(id: "predictions")
        source.data = .url(url)
